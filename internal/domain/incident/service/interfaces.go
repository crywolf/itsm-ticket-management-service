...
type IncidentService interface {
	...
	CreateIncident(ctx context.Context, channelID ref.ChannelID, actor user.Actor, params api.CreateIncidentParams) (ref.UUID, error)

	// GetIncident returns the incident with given ID from the repository
	GetIncident(ctx context.Context, channelID ref.ChannelID, actor user.Actor, ID ref.UUID) (incident.Incident, error)

	// ListIncidents returns the list of incidents from the repository
	ListIncidents(ctx context.Context, channelID ref.ChannelID, actor user.Actor) ([]incident.Incident, error)
}
